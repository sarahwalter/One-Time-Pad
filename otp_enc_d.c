...
{
	int listenSocketFD, establishedConnectionFD, portNumber, charsRead, pid, count, lmsg, lkey, status, i, yes;
	int childs[5];
		...
		if(count < 5) {
			if((pid = fork()) == 0) {
				...
				if (get_identifier(establishedConnectionFD, buffer) >= 0) {	/* Getting identifier */
					send_identifier(establishedConnectionFD, "DE");			/* Sending identifier */
					if(strcmp(buffer, "CE") == 0) {							/* Checking identifier */
